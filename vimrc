    ...
    function! s:RgSearch(...)
        ...
        while l:i < len(l:args)
            ...
            if (l:arg =~ "^'.*" || l:arg =~ '^".*') && !l:quote_found
                ...
                if (l:quote_type == "'" && l:arg =~ ".*'$") || (l:quote_type == '"' && l:arg =~ '.*"$')
            ...
            elseif l:quote_found && ((l:quote_type == "'" && l:arg =~ ".*'$") || (l:quote_type == '"' && l:arg =~ '.*"$'))
        ...
        let l:i = 0
        while l:i < len(l:args)
            if len(l:args[l:i]) > 0 && l:args[l:i][0] == '-'
...
" Define function to remove trailing spaces
